...
function filterChatConversations(value) {
    ...
    document.querySelectorAll(".chat-conv-item").forEach((item) => {
        const title =
            item.querySelector(".chat-conv-title")?.textContent.toLowerCase() ||
            "";
        const snippet =
            item
                .querySelector(".chat-conv-snippet")
                ?.textContent.toLowerCase() || "";
        item.style.display =
            !q || title.includes(q) || snippet.includes(q) ? "" : "none";
    });
}
...
function sendChatMessage() {
    ...
    const localImageUrls = pendingAttachments.map((f) =>
        URL.createObjectURL(f),
    );
    const userRow = document.createElement("div");
    userRow.className = "chat-msg-row user";
    userRow.innerHTML = `
    <div class="chat-avatar user">${window.AI_USER_INITIALS || "ME"}</div>
    ...
    fetch(window.AI_SEND_URL, {
        method: "POST",
        headers: { "X-CSRF-TOKEN": window.AI_CSRF_TOKEN, Accept: "text/html" },
        body: formData,
    })
        .then(async (res) => {
...
function deleteAiConversation(id) {
    if (
        !confirm(
            "Hapus percakapan ini? Semua pesan di dalamnya akan hilang permanen.",
        )
    )
        return;

    const url = window.AI_DELETE_URL_TEMPLATE.replace("__ID__", id);

    fetch(url, {
        method: "DELETE",
        headers: {
            "X-CSRF-TOKEN": window.AI_CSRF_TOKEN,
        ...
        .then((res) => {
            ...
            document
                .querySelector(`.chat-conv-item[data-conv-id="${id}"]`)
                ?.remove();

            // Kalau yang dihapus adalah percakapan yang sedang dibuka, pindah ke chat baru/index
            if (Number(id) === Number(window.AI_CONVERSATION_ID)) {
                window.location.href = window.AI_INDEX_URL;
                return;
            }
        })
        .catch((err) => {
...
function confirmAiAction(id) {
    ...
    const url = row?.dataset?.confirmUrl;
    if (!url) return;
    postAiActionCard(url, id);
}
